...
class ModelInterface:
    ...
    def load_from_state_dict(self, extra_weights, is_sd2=False):
        if is_sd2:
            mappings_to_use = PYTORCH_CKPT_MAPPING_SD2 
        else: 
            mappings_to_use = PYTORCH_CKPT_MAPPING

        models = {'text_encoder' : self.text_encoder, 
                  'diffusion_model' : self.diffusion_model, 
                  'decoder' : self.decoder, 
                  'encoder' : self.encoder}
        for module_name in models.keys():
            changed_weights = []
            current_weights = models[module_name].get_weights()
            for i, (key, perm) in enumerate(mappings_to_use[module_name]):
                if key in extra_weights:
                    new_weight = extra_weights[key]
                    
                    # Apply permutation if specified
                    if perm is not None:
                        new_weight = np.transpose(new_weight, perm)
                    
                    # Ensure the shapes match
                    if current_weights[i].shape == new_weight.shape:
                        current_weights[i] = new_weight
                        changed_weights.append(i)
                    else:
                        print(f"Shape mismatch for key {key}. Expected shape {current_weights[i].shape}, got {new_weight.shape}. Skipping this weight.")
            models[module_name].set_weights(current_weights)
            print("Changed %d weights for %s"%(len(changed_weights) , module_name))
